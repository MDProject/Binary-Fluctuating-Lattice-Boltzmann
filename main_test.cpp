...
int main(int argc, char* argv[]) {
  ...
  if(is_flate_interface){
    ...
    if(noiseSwitch){
      sprintf(plot_file_root_cstr, "%s/data_interface/lbm_data_shshan_alpha0_%.2f_xi_%.1e_size%d-%d-%d_continue/plt",
        root_path.c_str(), alpha0, kBT, dom_hi[0]-dom_lo[0]+1, dom_hi[1]-dom_lo[1]+1, dom_hi[2]-dom_lo[2]+1);
    }else{
  ...
  if(if_continue_from_last_frame){
    ...
    Real chk_temp = continueFromNonFluct? 0.: kBT;
  ...
  Print() << "LB initialized with alpha0 = " << alpha0 << " and kBT = " << kBT << '\n';
